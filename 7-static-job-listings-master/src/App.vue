<template>
  ...
  <div v-if="searchItems.length" class="container mx-auto flex items-center bg-white px-10 py-4 -mt-8 z-10 relative rounded-md gap-4 md:gap-4 flex-wrap">
    <div v-for="item in searchItems" :key="item" class="flex items-center">
      ...
      <div class="bg-desaturated-dark-cyan h-8 w-8 flex items-center justify-center rounded-r-md cursor-pointer" @click="removeFilter(item)">
    ...
    >
      <div class="flex flex-col md:flex-row gap-4">
        ...
        <div class="flex flex-col justify-between gap-4 md:gap-0">
          ...
          <h4 class="text-very-dark-grayish-cyan font-bold text-xl">{{ job.position }}</h4>
      ...
      <div class="flex gap-4 items-center md:justify-end mt-2 pt-4 border-t md:border-0">
        <p
          ...
          :key="item"
          class="flex items-center text-desaturated-dark-cyan font-bold bg-light-grayish-cyan h-8 px-3 rounded-sm cursor-pointer"
